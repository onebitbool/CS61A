...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if dice_num == 1:
            one_flag = True
        num_rolls -= 1
    return 1 if one_flag else dice_sum
    # END PROBLEM 1
...
def make_averaged(original_function, trials_count=1000):
    ...
    def compute(*args):
        ...
        count = trials_count
        while count > 0:
            amount += original_function(*args)
            count -= 1
        average = amount / trials_count
        return average
    return compute
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while num_rolls <= 10:
        ...
        while count < trials_count:
            amount += roll_dice(num_rolls, dice)
            count += 1
        average_score = amount / trials_count
        if average_score > max_score:
            max_score = average_score
            max_score_num_rolls = num_rolls
        num_rolls += 1
